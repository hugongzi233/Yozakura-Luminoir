...
nscr.exe是NScripter的库存版本（不是ONScripter！），因此不应用于翻译项目（它没有能力显示1字节字符）。它只是为了完整性。对于您的项目，您需要载最新的ONScript构建（或自己构建）。
您将在reference/中找到一个相对完整的命令参考作为examples/--中的几个示例，基本的0.txt示例仍在日语，但将在下一版本中进行翻译。有计划使用更新的命令和ONScripter添加到NScripter引用兼容性图表稍后也会显示。
